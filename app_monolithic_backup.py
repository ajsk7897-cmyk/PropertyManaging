    ...
    if not df_contracts_all.empty:
        ...
        for _, group in unique_groups.iterrows():
            ...
            if has_rf_this_month and (this_rent_base > 0 or this_maint_base > 0):
                # 렌트프리 절감액은 해당 월의 기본 임대료
                rf_records.append({
                    "자산명": asset,
                    "층": floor,
                    "업체명": company,
                    "절감 임대료": this_rent_base,
                    "절감 관리비": 0.0, # 보통 관리비는 렌트프리 대상 아님 (필요시 수정)
                    "통화": currency
                })
        ...
        with st.container():
            ...
            if update_mode == "🔄 계약 갱신":
                ...
                with st.container(border=True):
                    ...
                    with col_dl2:
                        if st.button("🚀 메일 발송", key="btn_email_new_entry", use_container_width=True, type="secondary"):
                            if to_email_new:
                                if not asset_name or not company_name:
                                    ...
                                    try:
                                        ...
                                        success, err = send_email_with_attachment(
                                            ...
                                            subject=f"[PM/AM] 신규 계약 체결 알림: {company_name}",
                                            body=f"신규 계약이 등록(또는 예정)되었습니다.\n자산명: {asset_name}\n업체명: {company_name}\n\n첨부된 엑셀 보고서를 확인해주세요.",
                                            file_bytes=report_bytes.getvalue(),
                                        ...
                                        st.warning(f"⚠️ 템플릿 처리 또는 메일 발송 실패: {email_err}")
                            else:
                                st.warning("이메일 주소를 입력해주세요.")
