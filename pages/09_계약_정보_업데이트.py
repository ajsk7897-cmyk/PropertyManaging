...
# ------------------
    ...
    with st.container():
        ...
        if update_mode == "🔄 계약 갱신":
        ...
        elif update_mode == "✨ 신규 계약":
            st.markdown("---")
            st.markdown("#### 📄 신규 계약 보고서 메일 발송")
            with st.container(border=True):
                ...
                with col_dl2:
                    if st.button("🚀 메일 발송", key="btn_email_new_entry", use_container_width=True):
                        if to_email_new:
                            if not asset_name or not company_name:
                                st.warning("자산명과 업체명을 먼저 입력해주세요.")
                            else:
                                try:
                                    ...
                                    ws = wb.active
                                    floor_str = ", ".join(sel_floors) if sel_floors else ""
                                    ws["B3"] = company_name
                                    ws["B4"] = asset_name
                                    ws["B5"] = floor_str
                                    ws["B6"] = start_date.strftime("%Y-%m-%d") if start_date else ""
                                    ws["B7"] = end_date.strftime("%Y-%m-%d") if end_date else ""
                                    ws["B8"] = deposit
                                    ws["B9"] = monthly_rent
                                    ...
                                    # 모듈화 환경에서는 send_email_with_attachment가 utils에 정의되어 있거나, 없으면 구현해야함
                                    # 여기서는 st.secrets를 직접 사용해서 보내는 원래 로직으로 복원
                                    import smtplib
                                    from email.message import EmailMessage
                                    
                                    if "email" in st.secrets:
                                        msg = EmailMessage()
                                        msg["Subject"] = f"[PM/AM] 신규 계약 체결 알림: {company_name}"
                                        msg["From"] = st.secrets["email"]["user"]
                                        msg["To"] = to_email_new
                                        msg.set_content(f"신규 계약이 등록(또는 예정)되었습니다.\n자산명: {asset_name}\n업체명: {company_name}\n\n첨부된 엑셀 보고서를 확인해주세요.")
                                        ...
                                        st.warning("⚠️ .streamlit/secrets.toml 에 이메일 설정이 없어 메일 발송이 생략되었습니다.")
